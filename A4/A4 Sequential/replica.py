...
class Replica():
    ...
    def processQueue(self):
        while True:
            if len(self.queue) > 0:
                self.log(self.queue[0].nacks)
                if self.queue[0].nacks == 0:
                    # processs
                    msg = heapq.heappop(self.queue)
                    if msg.getMessageType() == "_GetMessage_":
                        self.log(str(self.id)+" is processing get messae")
                        self.processGet(msg)
                    elif msg.getMessageType() == "_SetMessage_":
                    ...
                    self.broadcastAcks(self.queue[0].getAckMessage(self.id))
    ...
    def comparator(elem1, elem2):
        try:
            [client1,time1] = elem1.timeStamp.split(",")
            [client2,time2] = elem2.timeStamp.split(",")
            clientid1 = client1.split("C")[1]
            clientid2 = client2.split("C")[1]
            if time1 < time2:
                return  -1
            elif time2 <  time1:
                return 1
