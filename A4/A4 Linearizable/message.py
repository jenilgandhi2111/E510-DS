...
import time
class Message():
    def __init__(self,messageType,sender,senderPort,senderHost,toPort,toHost,meta=None,broadcast = False,nacks = 0):
        self.messageType = messageType
        self.sender = sender
        self.senderPort = senderPort
        self.senderHost = senderHost
...
class SetMessage(Message):
    def __init__(self,key,value,sender,senderPort,senderHost,toPort,toHost,broadcast,nacks) -> None:
        self.key = key
        self.value = value
        data = {"key":key,"value":value}
        super().__init__("_SetMessage_",sender,senderPort,senderHost,toPort,toHost,data,broadcast,nacks)
    ...
    def deserializeMessage(messageStr):
        ...
        return SetMessage(key,value,messageDict["sender"],messageDict["senderPort"],messageDict["senderHost"],messageDict["toPort"],messageDict["toHost"],messageDict["broadcast"],messageDict["nacks"])
...
class GetMessage(Message):
    ...
    def __init__(self,key,sender,senderPort,senderHost,toPort,toHost,broadcast,nacks) -> None:
        data = {"key":key}
        self.key = key
        super().__init__("_GetMessage_",sender,senderPort,senderHost,toPort,toHost,data,broadcast,nacks)
    
    def deserializeMessage(messageStr):
        messageDict = ast.literal_eval(messageStr)
        key = messageDict["meta"]["key"]
        return GetMessage(key,messageDict["sender"],messageDict["senderPort"],messageDict["senderHost"],messageDict["toPort"],messageDict["toHost"],bool(messageDict["broadcast"]),messageDict["nacks"])
...
class AcknowledgementMessage(Message):
    def __init__(self,sender,senderPort,senderHost,toPort,toHost,hsh,other):
        ...
        super().__init__("_ACK_",sender,senderPort,senderHost,toPort,toHost,meta,False,0)
